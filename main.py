...
def visualize_graph():
    Layout.algorithm(Graph.G,
                     selected_layout.get(),
                     node_size_entry.get(),
                     selected_shape.get(),
                     edge_width_entry.get(),
                     selected_label.get()
                     )
...
layout_label.grid(row=row, column=2, pady=10 , padx=10, sticky="E")
# Layout dropdown
layout_menu = ttk.Combobox(window, textvariable=selected_layout, state="readonly")
layout_menu["values"] = Layout.layout_options
layout_menu.grid(row=row, column=3, pady=10, padx=10, sticky="W")
row += 1
